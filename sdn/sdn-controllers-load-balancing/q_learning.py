...
def q_learning(controller_sets_Q, controllers_Q, k, switch_count, load_array):
    ...
    for n in range(0, iterations):
        ...
        save[n].append("Action = " + ["do nothing", "add 1 switch to {}".format(
            controllers_Q[C_i]), "remove 1 switch from {}".format(controllers_Q[C_i])][action])
        ...
        if action == 0:
            ...
            save[n] = save[n]  + "Source controller = {}, Destination controller = {}, Switch selected = {}, Number of flows on switch = {}".format(
                controller_src, controller_dest, switch, no_of_flows).split(",")
        ...
        save[n].append("Current State = {}".format(state))
        save[n].append("Next State = {}".format(next_state))
        ...
        if next_state in Q_matrix:
            ...
            save[n].append("Already present next state {} and Q-value {}".format(next_state, Q_matrix[next_state]))
        else:
